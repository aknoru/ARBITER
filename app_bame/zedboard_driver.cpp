...
#include <stdint.h>
#include <vector>
#include "sleep.h" // Xilinx Standalone API for usleep()
...
int main() {
    // -------------------------------------------------------------
    // Baremetal Memory Mapping: Direct Physical Pointer Casting
    // -------------------------------------------------------------
    volatile uint32_t* regs = (volatile uint32_t*)ADDR_WRAPPER;
    volatile uint32_t* fifo = (volatile uint32_t*)ADDR_FIFO;
